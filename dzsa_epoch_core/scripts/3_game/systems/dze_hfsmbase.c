void dze_fsmDebugPrint (string s)
{
 ...
 **/
class DZEHFSMBase<Class DZEStateBase, Class DZEEventBase, Class DZEActionBase, Class DZEGuardBase>
{
	protected ref DZEStateBase m_State; /// current fsm state
	protected DZEStateBase m_OwnerState; /// state that owns this fsm (or null if root)
	protected ref DZEStateBase m_InitialState; /// configurable initial state of the machine
	protected ref array<ref DZETransition<DZEStateBase, DZEEventBase, DZEActionBase, DZEGuardBase>> m_Transitions = new array<ref DZETransition<DZEStateBase, DZEEventBase, DZEActionBase, DZEGuardBase>>; /// fsm transition table
	protected bool m_HasCompletions = false;

	void DZEHFSMBase (DZEStateBase ownerState = NULL)
	{
	 ...
	 **/
	bool GetHierarchyPath (DZEStateBase state, out ref array<DZEStateBase> path)
	{
		DZEStateBase curr = state;
		while (curr)
		{
		  path.Insert(curr);
	 ...
	 **/
	void SetInitialState (DZEStateBase initial_state)
	{
	...
	void AddTransition (DZETransition<DZEStateBase, DZEEventBase, DZEActionBase, DZEGuardBase> t)
	{
		...
		//dze_fsmDebugPrint("[dzehfsm] +++ ow=" + this.GetOwnerState() + " this=" + this + " t=" + t + " state=" + t.m_srcState + "-------- event=" + t.m_event + "[G=" + t.m_guard +"]/A=" + t.m_action + " --------|> dst=" + t.m_dstState);
		if (t.m_event == NULL)
		{
	 ...
	 **/
	void Start (DZEEventBase initial_event = NULL)
	{
		dze_fsmDebugPrint("[dzehfsm] " + this.ToString() + "::Start(" + initial_event.ToString() + "), init_state=" + m_InitialState.ToString());
	 ...
	 **/
	void Terminate (DZEEventBase terminal_event = NULL)
	{
		dze_fsmDebugPrint("[dzehfsm] " + this.ToString() + "::Terminate(" + terminal_event.ToString() + ")");
		if (IsRunning())
		{
	...
	void Abort (DZEEventBase abort_event = NULL)
	{
		dze_fsmDebugPrint("[dzehfsm] " + this.ToString() + "::Abort(" + abort_event.ToString() + ")");
		if (IsRunning())
		{
	...
	protected ProcessEventResult ProcessAbortTransition (DZETransition<DZEStateBase, DZEEventBase, DZEActionBase, DZEGuardBase> t, DZEEventBase e)
	{
		dze_fsmDebugPrint("[dzehfsm] (local abort) state=" + t.m_srcState.ToString() + "-------- ABORT event=" + e.ToString() + "[G=" + t.m_guard.ToString() +"]/A=" + t.m_action.ToString() + " --------|> dst=" + t.m_dstState.ToString());
		...
		if (tmp == t.m_dstState.GetParentState())
		{
			...
			{
				dze_fsmDebugPrint("[dzehfsm] abort & terminating fsm: state=" + t.m_srcState.ToString() + " event=" + e.ToString());
	 ...
	 **/
	DZEStateBase FindAbortDestinationState (DZEEventBase e)
	{
		if (GetOwnerState())
			dze_fsmDebugPrint("[dzehfsm] SUB! " + GetOwnerState().Type().ToString() + "::FindAbortDestinationState(" + e.Type().ToString() + ")");
		else
			dze_fsmDebugPrint("[dzehfsm] root::FindAbortDestinationState(" + e.Type().ToString() + ")");

		// 1) look in submachine first (if any)
		if (m_State.HasFSM())
		{
			DZEHFSMBase<DZEStateBase, DZEEventBase, DZEActionBase, DZEGuardBase> a = m_State.GetFSM();
			DZEStateBase abort_dst = a.FindAbortDestinationState(e);
		...
		if (i == -1)
		{
			dze_fsmDebugPrint("[dzehfsm] abort event has no transition: src=" + m_State.ToString() + " e=" + e.Type().ToString());
			return NULL;
		}

		DZETransition<DZEStateBase, DZEEventBase, DZEActionBase, DZEGuardBase> t = m_Transitions.Get(i);
		return t.m_dstState;
	}
	 ...
	 * @return	FSM_ABORTED state machine has aborted
	 **/
	DZEStateBase ProcessAbortEvent (DZEEventBase e, out ProcessEventResult result)
	{
		if (GetOwnerState())
			dze_fsmDebugPrint("[dzehfsm] SUB! " + GetOwnerState().Type().ToString() + "::ProcessAbortEvent(" + e.Type().ToString() + ")");
		else
			dze_fsmDebugPrint("[dzehfsm] root::ProcessAbortEvent(" + e.Type().ToString() + ")");
		...
		if (m_State.HasFSM())
		{
			...
			DZEStateBase abort_dst = a.ProcessAbortEvent(e, subfsm_res);

			switch (subfsm_res)
			{
				case ProcessEventResult.FSM_OK:
				{
					dze_fsmDebugPrint("[dzehfsm] event processed by sub machine=" + m_State.ToString());
					result = subfsm_res;		// 1.1) submachine accepted event
					return NULL;
				}
				case ProcessEventResult.FSM_ABORTED:
				{
					dze_fsmDebugPrint("[dzehfsm] aborted sub machine=" + m_State.ToString());

					if (GetOwnerState() == abort_dst.GetParentState())
					{
						dze_fsmDebugPrint("[dzehfsm] aborted sub machine=" + m_State.ToString() + " & abort destination reached.");
						m_State = abort_dst;
				...
				{
					dze_fsmDebugPrint("[dzehfsm] aborted (but no transition) sub machine=" + m_State.ToString());
		...
		switch (res)
		{
			case ProcessEventResult.FSM_OK:
			{
				//dze_fsmDebugPrint("[dzehfsm] abort event processed by machine=" + m_State.ToString());
				return NULL; // machine accepted event
			}
			case ProcessEventResult.FSM_ABORTED:
			{
				dze_fsmDebugPrint("[dzehfsm] aborted sub machine=" + m_State.ToString() + " will fall-through to dst=" + t.m_dstState);
				return t.m_dstState; // store destination state for parent(s)
			}

			case ProcessEventResult.FSM_TERMINATED:
			{
				dze_fsmDebugPrint("[dzehfsm] aborted & terminated sub machine=" + m_State.ToString());
				break; // submachine has finished, look for local transitions from exited submachine
			}
			case ProcessEventResult.FSM_NO_TRANSITION:
			{	
	 ...
	 * @return	FSM_TERMINATED state machine has terminated
	 **/
	ProcessEventResult ProcessEvent (DZEEventBase e)
	{
		if (GetOwnerState())
			dze_fsmDebugPrint("[dzehfsm] SUB!::" + GetOwnerState().Type().ToString() + "::ProcessEvent(" + e.Type().ToString() + ")");
		else
			dze_fsmDebugPrint("[dzehfsm] root::ProcessEvent(" + e.Type().ToString() + ")");
		...
		if (i == -1)
		{
			dze_fsmDebugPrint("[dzehfsm] event has no transition: src=" + m_State.ToString() + " e=" + e.Type().ToString());
	...
	protected int FindFirstUnguardedTransition (DZEEventBase e)
	{
		DZEStateBase curr_state = m_State;
		Print("Find Unguarded - State: "+m_State.ToString());
		...
		for (int i = 0; i < count; ++i)
		{
			...
			{
				//dze_fsmDebugPrint("[dzehfsm] [" + i + "/" + count + "] *** matched! t=" + t + " state=" + t.m_srcState + "-------- event=" + t.m_event + "[G=" + t.m_guard +"]/A=" + t.m_action + " --------|> dst=" + t.m_dstState);
			...
			//else dze_fsmDebugPrint("[dzehfsm][" + i + "/" + count + "] ... matching  t=" + t + " state=" + t.m_srcState + "-------- event=" + t.m_event + "[G=" + t.m_guard +"]/A=" + t.m_action + " --------|> dst=" + t.m_dstState);
		}
		return -1;
	}
	...
	protected int FindFirstCompletionTransition ()
	{
		if (IsRunning())
		{
			...
			for (int i = 0; i < count; ++i)
			{
				...
				//dze_fsmDebugPrint("[dzehfsm] (local) matching state=" + t.m_srcState + "-------- event=" + t.m_event + "[G=" + t.m_guard +"]/A=" + t.m_action + " --------|> dst=" + t.m_dstState);
	 ...
	 **/
	protected ProcessEventResult ProcessLocalTransition (DZETransition<DZEStateBase, DZEEventBase, DZEActionBase, DZEGuardBase> t, DZEEventBase e)
	{
		dze_fsmDebugPrint("[dzehfsm] (local) state=" + t.m_srcState.ToString() + "-------- event=" + e.ToString() + "[G=" + t.m_guard.ToString() +"]/A=" + t.m_action.ToString() + " --------|> dst=" + t.m_dstState.ToString());
		...
		{
			dze_fsmDebugPrint("[dzehfsm] terminating fsm: state=" + t.m_srcState.ToString() + " event=" + e.ToString());
	 ...
	 * @return	FSM_OK or FSM_TERMINATED
	 **/
	protected ProcessEventResult LocalTransition (int i, DZEEventBase e)
	{
		DZETransition<DZEStateBase, DZEEventBase, DZEActionBase, DZEGuardBase> t = m_Transitions.Get(i);
	...
	protected ProcessEventResult ProcessCompletionTransitions ()
	{
		...
		{
			DZETransition<DZEStateBase, DZEEventBase, DZEActionBase, DZEGuardBase> row = m_Transitions.Get(completionIdx);
			ProcessEventResult res;
			if (row.m_dstState != NULL)
			{
...
/** cross-hierarchy transitions (for future use):
	...
	{
		//for (int j = 0; j < src_path.Count(); ++j) dze_fsmDebugPrint("[dzehfsm] curr state src_path[" + j + "] = " + src_path[j].ToString());
		//for (int k = 0; k < dst_path.Count(); ++k) dze_fsmDebugPrint("[dzehfsm] next state dst_path[" + k + "] = " + dst_path[k].ToString());
		...
		//for (int je = 0; je < src_path.Count(); ++je) dze_fsmDebugPrint("[dzehfsm] curr state src_path[" + je + "] = " + src_path[je].ToString());
		//for (int ke = 0; ke < dst_path.Count(); ++ke) dze_fsmDebugPrint("[dzehfsm] next state dst_path[" + ke + "] = " + dst_path[ke].ToString());
	}
	protected ProcessEventResult HierarchicTransition (int i, DZEEventBase e)
	{
		dze_fsmDebugPrint("[dzehfsm] { " + this.ToString() + "::HierarchicTransition(" + e.Type().ToString() + ")");
		...
		if (GetHierarchyPath(t.m_srcState, src_path))
		{
			if (t.m_dstState == NULL)
			{
				...
				//for (int ex = 0; ex < src_path.Count(); ++ex) { dze_fsmDebugPrint("[dzehfsm] exiting level=" + ex); src_path[ex].OnExit(e); }
				//for (int en = dst_path.Count(); en --> 0;)
				//for (int en = 0; en < dst_path.Count(); ++en) { dze_fsmDebugPrint("[dzehfsm] entering level=" + en); int ii = dst_path.Count() - 1 - en; dst_path[ii].OnEntry(e); }
				dze_fsmDebugPrint("[dzehfsm] } " + this.ToString() + "::HierarchicTransition(" + e.Type().ToString() + ") --> terminated");
			...
			else if (GetHierarchyPath(t.m_dstState, dst_path))
			{
				...
				dze_fsmDebugPrint("[dzehfsm] } " + this.ToString() + "::HierarchicTransition(" + e.Type().ToString() + ") --> OK");
				return ProcessEventResult.FSM_OK;
			}
			else
			{
		...
		dze_fsmDebugPrint("[dzehfsm] } " + this.ToString() + "::HierarchicTransition(" + e.Type().ToString() + ") --> no transition");
		return ProcessEventResult.FSM_NO_TRANSITION;
	}
*/
